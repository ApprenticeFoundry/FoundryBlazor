...
public interface IFoCollection
{
    ...
    List<FoBase> AllValues();
    List<FoBase> AllValuesOfType(Type type);
    void ExecuteForEachItem(Action<FoBase> applyClause);
...
public class FoCollection<T>: IFoCollection where T : FoBase
{
    ...
    public List<FoBase> AllValues()
    {
        return this.members.Values.Cast<FoBase>().ToList();
    }

    public List<FoBase> AllValuesOfType(Type type)
    {
        return this.members.Values.Where(item => item.GetType() == type || item.GetType().IsSubclassOf(type)).Cast<FoBase>().ToList();
    }
    ...
    public void ExecuteForEachItem(Action<FoBase> applyClause)
    {
