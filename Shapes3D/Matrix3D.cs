...
namespace FoundryBlazor.Shape;
 ...
 public class Matrix3D 
 {
    ...
    private static readonly Queue<Matrix3D> cashe = new Queue<Matrix3D>();
    public static Matrix3D NewMatrix()
    {
        if (cashe.Count == 0)
            return new Matrix3D();

        //$"Recycle Matrix3D {cashe.Count}".WriteInfo();
    ...
    public static Matrix3D? SmashMatrix(Matrix3D? source)
    {
       ...
       // $"Smash Matrix3D {cashe.Count}".WriteNote();
        return null;
    }
    ...
    public Matrix3D Copy(Matrix3D matrix) 
    {
        a = matrix.a;
        b = matrix.b;
        c = matrix.c;
    ...
    public Matrix3D Append(double a, double b, double c, double d, double tx, double ty) 
    {
        var a1 = this.a;
        var b1 = this.b;
        var c1 = this.c;
    ...
    public Matrix3D Prepend(double a, double b, double c, double d, double tx, double ty) 
    {
        var a1 = this.a;
        var c1 = this.c;
        var tx1 = this.tx;
    ...
    public Matrix3D Set(double a, double b, double c, double d, double tx, double ty) 
    {
        this.a = a;
        this.b = b;
        this.c = c;
    ...
    public Matrix3D AppendTransform(double x, double y, double scaleX, double scaleY, double rotation, double regX, double regY) {

        var r = rotation * Matrix3D.DEG_TO_RAD;
        var cos = Math.Cos(r);
        var sin = Math.Sin(r);
    ...
    public Matrix3D AppendTransformWithSkey(double x, double y, double scaleX, double scaleY, double rotation, double skewX, double skewY, double regX, double regY) {
        ...
        if (skewX != 0.0 || skewY != 0.0) {
            ...
            skewX *= Matrix3D.DEG_TO_RAD;
            skewY *= Matrix3D.DEG_TO_RAD;
    ...
    public Matrix3D PrependTransform(double x, double y, double scaleX, double scaleY, double rotation, double skewX, double skewY, double regX, double regY) {
    ...
    {
        var ang = angle * Matrix3D.DEG_TO_RAD;
    ...
    public Matrix3D Skew(double skewX, double skewY) {
        var skewx = skewX * Matrix3D.DEG_TO_RAD;
        var skewy = skewY * Matrix3D.DEG_TO_RAD;
