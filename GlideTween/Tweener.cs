...
namespace Unglide
{
    ...
    public partial class Tween
    {
        public class TweenerImpl
        {
            ...
            {
                //$"Update(float {secondsElapsed}) ".WriteLine(ConsoleColor.DarkBlue);
            ...
            private void ApplyAll(Action<Tween> action)
            {
                if (_tweens.Count == 0 )
                {
                   // $"the tweens are empty ".WriteLine(ConsoleColor.DarkBlue);
                    return;
                }
            ...
            private void ApplyBulkControl(object[] targets, Action<Tween> action)
            {
                ...
                {
                    if (_tweens.TryGetValue(target, out List<Tween>? list))
                    {
                        foreach (var glide in list)
                        {
                            action(glide);
