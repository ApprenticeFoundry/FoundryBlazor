...
public class UnglideInfo
{
    ...
    public object? Value
    {
        ...
        set
        {
            ...
            if (_isNumeric)
            {
                ...
                if (AnyEquals(type, NumericTypes) && type != null)
    ...
    public UnglideInfo(object target, string property, bool writeRequired = true)
    {
        ...
        var info = targetType.GetTypeInfo();

        _field = info.GetFields().FirstOrDefault(f => string.Equals(property, f.Name) && !f.IsStatic);

        var writeable = info.GetProperties().FirstOrDefault(p => string.Equals(property, p.Name) &&  p.CanRead && p.CanWrite);
        var readable = info.GetProperties().FirstOrDefault(p => string.Equals(property, p.Name) && p.CanRead);

        _prop = writeRequired ? writeable : readable;
